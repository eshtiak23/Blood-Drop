    ...
    <div className="container" style={{ padding: "32px 20px", maxWidth: 700 }}>
      ...
      <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 8 }}>
        {notifications.length === 0 ? (
          <div className="empty-state"><div className="empty-state-icon"><Bell size={28} color="var(--red)" /></div><div className="empty-state-title">No notifications</div><div className="empty-state-desc">You're all caught up!</div></div>
        ) : notifications.map((n) => (
          <div key={n._id} className="card" style={{ padding: 16, background: !n.isRead ? "var(--red-light)" : undefined, borderLeft: !n.isRead ? "3px solid var(--red)" : undefined }}>
            <div style={{ display: "flex", gap: 12, alignItems: "start" }}>
              ...
              <div style={{ display: "flex", gap: 4 }}>
                ...
                <button className="btn btn-ghost btn-sm" onClick={() => remove(n._id)} style={{ padding: 6, color: "var(--red)" }}><Trash2 size={14} /></button>
              </div>
            </div>
          </div>
        ))
